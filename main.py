...
LOOP_INTERVAL = 120
RECIPIENT_ID = "6a29eff3373155f11f0b60d6"
TIP_CURRENCY = "mm2"
TIP_CRYPTO_THRESHOLD = 1
TIP_CRYPTO_LARGE = 0.5
...
def load_accounts(path="tokens.txt"):
    if not os.path.exists(path):
        print(f"[!] tokens.txt not found!")
        return []
...
def send_embed(title, description, color, fields=None, thumbnail=None):
    embed = {
        ...
        "footer": {"text": "RBXChance Auto-Joiner • Made with love by n7h8"}
    }
...
def manual_tip_mm2(accounts, recipient_id):
...
def manual_tip_crypto(accounts, recipient_id):
...
def main():
    ...
    if not accounts:
        print("[!] No accounts in tokens.txt")
        return
